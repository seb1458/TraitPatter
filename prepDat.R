...
NMDS <- select(NMDS, -c(stage1:stage4), -c(drift_low:drift_high), -c(life1:life2)) 

NMDS <- NMDS %>%
  mutate(total_sum = rowSums(.[4:ncol(NMDS)])) %>%
  filter(total_sum == 8) %>%
  select(-total_sum)
...
MCA <- select(MCA, -Drift, -Life.Duration, -Aquatic.Stages)
...
SOM <- select(SOM, -c(drift, life, aquatic_stages))
