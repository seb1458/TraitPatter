...
# ---- EUR: Aggregate on family level ----
# Get list of genera frequencies
fam_freq <- as.data.frame(table(EUR$order))

# Merge with European database
EUR <- merge(EUR, fam_freq, by.x = "order", by.y = "Var1", all.x = TRUE, sort = FALSE)

EUR <- select(EUR, order, everything())
...
EUR_agg <- EUR_agg %>%
  ...
  # Add regions column
  mutate(region = "EUR") %>%
  select(order, region, everything())

EUR_agg <- EUR_agg[!is.na(EUR_agg$order), ]
...
# ---- NAM: Aggregate on order level ----
# Get list of genera frequencies
fam_freq <- as.data.frame(table(NAM$Order))

# Merge with European database
NAM <- merge(NAM, fam_freq, by.x = "Order", by.y = "Var1", all.x = TRUE, sort = FALSE)

NAM <- select(NAM, Order, everything())
...
# ---- NAM: Aggregate on order level ----
# Get list of genera frequencies
fam_freq <- as.data.frame(table(AUS$Order))

# Merge with European database
AUS <- merge(AUS, fam_freq, by.x = "Order", by.y = "Var1", all.x = TRUE, sort = FALSE)

AUS <- select(AUS, Order, everything())
